<template>
  <q-page class="row q-pa-md q-pt-xl bg-sky">
    <p class="pagetit">徐汇区原位水质监控决策分析</p>
    <q-tabs class="tab">
      <q-route-tab
        to="/waterAnalysis"
        exact
...
export default {
  ...
  methods: {
    ...
    shapeHeatData (EleRelation) {
      let heatmapValue = []
      let heatmapData = EleRelation.value
      heatmapData.forEach((d, i) => {
        d.forEach((m, j) => {
          heatmapValue.push([j, i, m])
        })
      })
      return heatmapValue
    },
    shapeRiverData (AbData) {
      let riverValue = []
      let riverData = AbData.value
      let label = AbData.yfields
      // if (this.showTest || this.showTest1) {
      //   console.log(label)
      //   if (!this.showTest) {
      ...
      riverData.forEach((d, i) => {
        d.forEach((m, j) => {
          if (label[i] === '序列异常') {
            riverValue.push([j, m, '时空异常'])
          } else {
            riverValue.push([j, m, label[i]])
          }
        })
      })
      return riverValue
